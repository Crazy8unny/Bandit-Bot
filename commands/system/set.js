...
class SetCMD extends Command {
  ...
  async run(message, [action, key, ...value], level) { // eslint-disable-line no-unused-vars
    ...
    // if (!(await this.client.settings.doc(message.guild.id).get()).exist) this.client.settings.doc(message.guild.id).set({}));
    ...
    if (action === "edit") {
      ...
      if (!(await this.client.settings.doc(message.guild.id).get()).exist) this.client.settings.set(message.guild.id, {});
