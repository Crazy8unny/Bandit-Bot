...
class BanditBot extends Client {
  constructor (options) {
    ...
    this.servers = this.db.collection("lastThread").doc("Servers").get().then(Servers => {return Servers});
...
const init = async () => {
  ...
  klaw("./containsCommands").on("data", (item) => {
    const cmdFile = path.parse(item.path);
    if (!cmdFile.ext || cmdFile.ext !== ".js") return;
    const response = client.loadContainsCommand(cmdFile.dir, `${cmdFile.name}${cmdFile.ext}`);
    if (response) client.logger.error(response);
  });
